...
export class DataStore {
  ...
  constructor(rootHandle = null) { this.rootHandle = rootHandle; }
  ...
  async save(sample, stage = 'raw') {
    ...
    if (!this.rootHandle?.getDirectoryHandle) throw new Error('Selecciona una carpeta persistente de data/ antes de guardar.');
  ...
  async list(stage = 'raw') {
    if (!this.rootHandle?.getDirectoryHandle) throw new Error('Selecciona una carpeta persistente de data/ antes de leer.');
